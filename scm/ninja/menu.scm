...
(define (make-ninja-menu title)      
  (make <ninja-menu> #:title title))
(define (make-ninja-ms-menu title) 
  (make <ninja-ms-menu> #:title title))
...
(define-method (add-item! (menu <ninja-menu>) name short action allowed)
  (let ((item (make-ninja-menu-item menu name short (or action (string->symbol name)) allowed)))
...
(define-method (draw-menu-generic (menu <ninja-menu>) (draw-item <procedure>) scr)
  (let ((mx (getmaxx scr))
    ...
    (clear scr)
    (refresh scr)
    (addchstr scr (bold (get-title menu)) #:x 0 #:y 0)
...
(define-method (draw-menu (menu <ninja-menu>) scr)
  (let ((draw-item (λ (item y x)
                      (addstr scr (format #f " [~a]~:[*~; ~]~a" (get-short item) (is-allowed? item) (get-name item)) #:y y #:x x))))
    (draw-menu-generic menu draw-item scr)))

(define-method (draw-menu (menu <ninja-ms-menu>) scr)
  (let ((draw-item (λ (item x y)
                      (addstr scr (format #f " [~a] ~:[+~;-~]~:[*~: ~]~a" (get-short item) (memq (get-short item) (selected menu)) (is-allowed? item) (get-name item)) #:y y #:x x))))
    (draw-menu-generic menu draw-item scr)))
